/// Demo / simulation mode — entirely offline, no WS, no QuestDB.
/// All simulation code lives here, isolated from the live path.
...
pub fn run() {
    info!("=== Demo Mode: Offline Simulation ===");
    ...
    if let Some(mut btc_tracker) = manager.active_markets.get_mut("btc-updown-5m-1772334000") {
        ...
        for i in 1..=10_000 {
            ...
            if i % 100 == 0 {
                ...
                btc_tracker
                    .microstructure
                    .update_tick(now_micros(), i as u64);
            }
        }
        ...
        let book_imbalance = imbalance::imbalance(&btc_tracker.book);
        let microprice = imbalance::microprice(&btc_tracker.book);
        info!("Current BTC Imbalance (Top 10): {:.4}", book_imbalance);
        info!("Current BTC Microprice: {:?}", microprice);
    }

    // 3. Market Summary computation test (resolution)
    info!("Simulating 5-minute market resolution...");
    let end_ts = start_ts + 300_000_000;
